...
//bentuk pelanggaran
  ...
  else if($akh==md5('updatepassword')){
    $smk->updatepassword($con,$_POST['nama'],$_POST['username'],$_POST['password']);
  }
...
//siswa
    ...
    for ($i=2; $i<=$jumlah_baris; $i++){
        ...
        $tl  = $data->val($i, 6);
        $c_siswa=random(9);
        if($c_kelas != "" && $nisn != "" && $nama != "" && $c_siswa != ""){
            mysqli_query($con,"INSERT into siswa SET c_siswa='$c_siswa',c_kelas='$c_kelas',nisn='$nisn',nama='$nama',jk='$jk',alamat='$alamat',tl='$tl'");
            $berhasil++;
        }
    }
    ...
    session_start();
    clearstatcache();
    header('location:../../upload-siswa/'.$berhasil.'');  
  }
